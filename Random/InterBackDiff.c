...
int solveBackward()
{
	printf("Interpolation using Newton's Backward Difference Method:\n\n");
	int n;
	do
	{
	...
	printf("\nWARNING: Two consecutive values of x must be equidistant and increasing\n");
	double x[n];
	double f[n];
	double X;
	int i,j;
	...
	printf("\n\nEnter the value of point to be found using interpolation (must be less than %lf): ",x[n-1]);
	scanf("%lf",&X);
	double h=x[1]-x[0];
	double s=(X-x[n-1])/h;
	double df[n-1][n-1];
